/**
 * DBTraders — Full Deriv Trading App
 *
 * Auth flow:
 *   1. OAuth redirect → https://auth.deriv.com/oauth2/auth
 *   2. Exchange code → POST https://auth.deriv.com/oauth2/token
 *   3. Fetch OTP    → POST https://api.derivws.com/trading/v1/options/accounts/{ACCOUNT_ID}/otp
 *   4. Open auth WS → wss from otp response
 *
 * Public WS: wss://api.derivws.com/trading/v1/options/ws/public
 *
 * Messages (JSON over WS):
 *   active_symbols, contracts_for, contracts_list, proposal, buy, balance, portfolio
 */
...
// Replace with your actual OAuth client_id / redirect_uri
...
async function exchangeCode(code) {
  ...
  return res.json(); // { access_token, account_id, ... }
}
...
// ─── Main App ─────────────────────────────────────────────────────────────────
export default function DBTraders() {
  // Auth state
  const [authState, setAuthState] = useState("idle"); // idle | loading | authed | error
  const [authError, setAuthError] = useState(null);
  const [account, setAccount]   = useState(null); // { access_token, account_id }
  const [authWsUrl, setAuthWsUrl] = useState(null);
  ...
  // UI
  const [tab, setTab]                 = useState("market"); // market | trade | portfolio
  ...
  const authWs = useWebSocket(null, {
    ...
    onMessage: (msg) => {
      ...
      if (msg.msg_type === "portfolio") {
        setPortfolio(msg.portfolio?.contracts || []);
      }
  ...
  // ── Current tick ──────────────────────────────────────────────────────────
  const currentTick = selectedSymbol && ticks[selectedSymbol]
    ? ticks[selectedSymbol].slice(-1)[0]
    : null;
  const prevTick = selectedSymbol && ticks[selectedSymbol]?.length > 1
    ? ticks[selectedSymbol].slice(-2)[0]
    : null;
  ...
  return (
    <div style={s.app}>
      ...
      <aside style={s.sidebar}>
        <div style={s.logo}>
          <span style={s.logoMark}>DB</span>
          <span style={s.logoText}>Traders</span>
        </div>
        ...
        <div style={s.sideNav}>
          {[
            ...
            <button key={key} style={clsx()} onClick={() => setTab(key)}
              className={tab === key ? "nav-active" : ""}
              data-active={tab === key}
              onMouseEnter={e => { if (tab !== key) e.currentTarget.style.background = "#1e293b"; }}
              ...
              style={{
                ...
                borderLeft: tab === key ? "2px solid #22d3ee" : "2px solid transparent",
              }}>
          ...
          {authState === "authed" ? (
            <>
              ...
              <div style={s.balanceDisplay}>
                ...
                <span style={{ color: "#f1f5f9", fontSize: 22, fontFamily: "monospace", fontWeight: 700 }}>
            ...
            <button style={s.loginBtn} onClick={redirectToOAuth}>
                  ...
                  <div key={sym.symbol}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      ...
                      <div style={{
                        ...
                        fontWeight: 700,
                        color: up ? "#22c55e" : dn ? "#f87171" : "#94a3b8",
                        minWidth: 80,
                        textAlign: "right",
                      }}>
                        {last != null ? fmt(last) : "—"}
              ...
              <div style={s.tradePanel}>
                ...
                <div style={s.livePriceBox}>
                  ...
                  <span style={{
                    ...
                    fontFamily: "monospace",
                    fontWeight: 800,
                    color: tickUp ? "#22c55e" : tickDown ? "#f87171" : "#f1f5f9",
                ...
                <select style={s.select} value={selectedContract?.contract_type || ""}
                  onChange={e => {
                    const c = contracts.find(x => x.contract_type === e.target.value);
                    setSelectedContract(c || null);
                  }}>
                  {contracts.map(c => (
                    <option key={c.contract_type} value={c.contract_type}>
                      {c.contract_display_name || c.contract_type}
                    </option>
                  ))}
                </select>
                ...
                <div style={s.row2}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input style={s.input} type="number" min="1" value={stake}
                      onChange={e => setStake(e.target.value)} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={s.fieldLabel}>Duration</label>
                    <div style={{ display: "flex", gap: 6 }}>
                      <input style={{ ...s.input, flex: 1 }} type="number" min="1" value={duration}
                        onChange={e => setDuration(e.target.value)} />
                      <select style={{ ...s.select, width: 70 }} value={durationUnit}
                ...
                <button style={{ ...s.primaryBtn, opacity: authState !== "authed" ? 0.4 : 1 }}
                  disabled={authState !== "authed" || proposalLoading}
              ...
              <div style={s.proposalPanel}>
                {proposal ? (
                  <div style={s.proposalCard}>
                    ...
                    <button style={{ ...s.buyBtn, opacity: buyLoading ? 0.5 : 1 }}
                      disabled={buyLoading} onClick={buyContract}>
                  ...
                  <div style={s.emptyProposal}>
                    ...
                    <p style={{ color: "#475569", margin: "12px 0 0", fontSize: 14 }}>
                      Select an asset and contract, then get a price quote.
                    </p>
                  </div>
                )}
                ...
                {portfolio.map(c => (
                  <div key={c.contract_id} style={s.tableRow}>
                    ...
                    <span style={{ color: "#64748b", fontSize: 12 }}>
                      {c.expiry_time ? new Date(c.expiry_time * 1000).toLocaleTimeString() : "—"}
                    </span>
                  </div>
                ))}
              </div>
      ...
      {toast && (
        <div style={{
          ...s.toast,
          background: toast.ok ? "#052e16" : "#1f0a0a",
          borderColor: toast.ok ? "#22c55e" : "#f87171",
